...
SELECT location, population, MAX(total_cases) AS highest_infection_count, MAX((total_cases/population)*100) AS highest_case_percentage FROM PortfolioProject..CovidDeaths
WHERE continent IS NOT NULL
...
SELECT dea.continent, dea.location, dea.date, dea.population, MAX(vac.total_vaccinations) AS highest_vaccinations_count
...
SELECT location, SUM(CAST(new_deaths AS INT)) AS total_new_death
FROM PortfolioProject..CovidDeaths
WHERE continent IS NULL
AND location NOT IN ('World', 'European Union', 'International')
GROUP BY location
ORDER BY total_new_death DESC
...
-- 7.
SELECT location, population, date, MAX(total_cases) AS highest_infection_count, MAX((total_cases/population)*100) AS highest_case_percentage FROM PortfolioProject..CovidDeaths
WHERE continent IS NOT NULL
